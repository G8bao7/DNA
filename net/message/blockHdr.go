...
func (msg headersReq) Handle(node Noder) error {
	...
	log.Trace("The Start Hash ", startHash, " stopHash ", stopHash, " cnt ", cnt)
	if err != nil {
		return err
	}
...
func SendSyncHeaders(node Noder) {
	log.Trace()
...
func GetHeadersFromHash(startHash common.Uint256, stopHash common.Uint256) ([]ledger.Header, uint32, error) {
	...
	if startHash == empty {
		if stopHash == empty {
			...
			blkStop, err := ledger.DefaultLedger.GetBlockWithHash(stopHash)
			if err != nil {
				return nil, 0, err
			}
			stopHeight = blkStop.Blockdata.Height
